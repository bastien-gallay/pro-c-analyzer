...
class TestAnalyzeWithProgress:
    ...
    def test_analyze_directory_case_insensitive_multiple_patterns(self, tmp_path, simple_proc_source):
...
class TestCaseInsensitiveMatching:
    ...
    def test_find_files_case_insensitive_all_files_found(self, tmp_path, simple_proc_source):
        ...
        files = find_files_case_insensitive(
            tmp_path, 
            ["*.PC", "*.SC", "*.INC"], 
            recursive=True
        )
        ...
        filenames = {f.name for f in files}
        expected_files = {
            "file1.PC", "file2.pc", "file3.PC", "file4.sc", 
            "file5.INC", "file6.inc", "file7.PC", "file8.SC"
        }
        assert filenames == expected_files, f"Fichiers manquants: {expected_files - filenames}, fichiers inattendus: {filenames - expected_files}"
