...
class CursorAnalyzer:
    ...
    DECLARE_CURSOR = re.compile(
        r"EXEC\s+SQL\s+DECLARE\s+(\w+)\s+CURSOR\s+FOR\s+(.*?);", re.IGNORECASE | re.DOTALL
    )

    OPEN_CURSOR = re.compile(r"EXEC\s+SQL\s+OPEN\s+(\w+)", re.IGNORECASE)

    FETCH_CURSOR = re.compile(r"EXEC\s+SQL\s+FETCH\s+(\w+)", re.IGNORECASE)

    CLOSE_CURSOR = re.compile(r"EXEC\s+SQL\s+CLOSE\s+(\w+)", re.IGNORECASE)

    # Pattern pour curseurs dynamiques
    PREPARE_STMT = re.compile(r"EXEC\s+SQL\s+PREPARE\s+(\w+)\s+FROM", re.IGNORECASE)

    DECLARE_DYNAMIC = re.compile(
        r"EXEC\s+SQL\s+DECLARE\s+(\w+)\s+CURSOR\s+FOR\s+(\w+)", re.IGNORECASE
    )
    ...
    SQLCODE_CHECK = re.compile(r"(sqlca\.sqlcode|SQLCODE)\s*(==|!=|<|>|<=|>=)", re.IGNORECASE)
