...
class MemoryAnalyzer:
    ...
    DANGEROUS_FUNCTIONS = {
        ...
        "strcpy": (
            "strncpy/strlcpy",
            "WARNING",
            "strcpy() ne vérifie pas la taille du buffer destination",
        ),
        "strcat": (
            "strncat/strlcat",
            "WARNING",
            "strcat() ne vérifie pas la taille du buffer destination",
        ),
        "sprintf": (
            "snprintf",
            "WARNING",
            "sprintf() ne vérifie pas la taille du buffer destination",
        ),
        "vsprintf": (
            "vsnprintf",
            "WARNING",
            "vsprintf() ne vérifie pas la taille du buffer destination",
        ),
        ...
        "realpath": (
            "realpath avec buffer NULL",
            "INFO",
            "realpath() avec buffer fixe peut overflow",
        ),
    ...
    def _check_sizeof_pointer(self, source: str) -> None:
        ...
        """
        pattern = re.compile(
            r"(malloc|calloc)\s*\([^)]*sizeof\s*\(\s*(\w+)\s*\)\s*\)", re.IGNORECASE
        )
